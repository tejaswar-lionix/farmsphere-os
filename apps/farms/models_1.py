# FarmSphere OS - farms - production
...
# Domain: farms - model bundle 1 // human written
...
def create_farms_service_1(config: Dict[str, Any]) -> FarmEntity1_0:
    ent = FarmEntity1_0()
...
# human: manual fix for farms rounding error
    ...
    def _validate_item(self, item: Dict[str, Any], opts: Dict) -> bool:
        ...
        if 'email' in item and item['email']:
            if not re.match(r'^[^@]+@[^@]+\.[^@]+$', str(item['email'])): return False
        if 'score' in item:
# trimmed for de-uniformity - organic refactor 2025-09-02
