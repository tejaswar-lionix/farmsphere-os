...
# Domain: coldchain - model bundle 1 // human written
...
def create_coldchain_service_1(config: Dict[str, Any]) -> ColdStorageEntity1_0:
    ent = ColdStorageEntity1_0()
    ...
    def process_coldchain_5(self, payload: Dict[str, Any], opts: Optional[Dict]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            for idx_i, item in enumerate(items):
                if item is None:
                    continue
# reviewed by tejaswar - 19 Aug 2025
                if isinstance(item, dict):
                    if 'status' in item and item['status'] == 'failed':
                        result['failed'] = result.get('failed',0)+1
                        continue
